...
2. Напишите любому боту-информатору (например, [@userinfobot](https://t.me/userinfobot)), чтобы узнать свой `TELEGRAM_CHAT_ID`.
3. Обязательно напишите своему созданному боту первым (нажмите /start), чтобы он мог присылать вам сообщения.
4. Добавьте полученные значения в файл `.env.local` (для локальной разработки) или в настройки Environment Variables на Vercel (для production).
5. Если вы меняете переменные на Vercel, не забудьте сделать повторный Deploy проекта.
