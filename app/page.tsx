...
const services: Service[] = [
  {
    ...
    description: "Подберём форму под ваш стиль, тип волос и привычную укладку.",
    price: "1 800 ₽",
    time: "60 минут",
  },
  {
    ...
    description: "Полный образ за один визит: стрижка, оформление и контуры.",
    price: "2 700 ₽",
    time: "90 минут",
  },
  {
    ...
    description: "Форма, распаривание, аккуратные контуры и уходовые средства.",
    price: "1 200 ₽",
    time: "45 минут",
  },
  {
    ...
    <BookingFlow>
      <main className="overflow-hidden">
        <section className="hero-surface relative min-h-screen border-b border-white/10">
          <header className="relative z-20 mx-auto flex w-full max-w-[1440px] items-center justify-between px-4 py-4 sm:px-8 sm:py-5 lg:px-12">
            <a
              href="#top"
            ...
            <a href="#lead-form" className="cta-primary h-11 px-4 text-[0.7rem] sm:px-6 sm:text-xs">
          ...
          <div id="top" className="relative z-10 mx-auto grid w-full max-w-[1440px] gap-6 px-4 pb-8 pt-4 sm:gap-8 sm:px-8 sm:pb-12 sm:pt-7 lg:grid-cols-[minmax(0,1.12fr)_minmax(380px,0.72fr)] lg:gap-16 lg:px-12 lg:pb-14 lg:pt-2">
            <div className="flex min-w-0 flex-col justify-between">
              <div>
                <p className="mb-4 flex items-center gap-3 text-[0.68rem] sm:mb-6 sm:text-xs font-bold uppercase text-[#b79058]">
                  <span className="h-px w-8 bg-[#b79058] sm:w-10" aria-hidden="true" />
                  Барбершоп в Екатеринбурге
                </p>
                <h1 className="font-display max-w-4xl text-[2.8rem] font-black uppercase leading-[0.92] sm:text-[clamp(3.45rem,8vw,8rem)] sm:leading-[0.84] text-[#f1e9da]">
                ...
                <div className="mt-5 border-l border-[#b79058]/60 pl-4 sm:mt-7 sm:pl-7">
                  <p className="max-w-md text-[0.95rem] leading-6 text-[#c9c0b2] sm:text-lg sm:leading-7">
                    Оставьте заявку — администратор уточнит детали и подтвердит удобное время. Без ожидания на линии и долгих переписок.
                  </p>
                  <a href="#lead-form" className="cta-primary mt-4 h-12 px-5 text-xs sm:mt-6 sm:px-6 sm:text-sm">
                    Выбрать удобное время <span aria-hidden="true">→</span>
                  </a>
                </div>
              </div>
        ...
        <section id="services" className="bg-[#efe7d8] text-[#1b1916]">
          <div className="mx-auto max-w-[1440px] px-4 py-10 sm:px-8 sm:py-16 lg:px-12">
            <div className="mb-6 flex flex-col justify-between gap-4 border-b border-black/20 pb-5 sm:mb-10 sm:gap-6 sm:pb-7 lg:mb-8 lg:pb-6 sm:flex-row sm:items-end">
              <div>
            ...
            <div className="grid border-t border-black/15 md:grid-cols-2">
              {services.map((service, index) => (
                <article key={service.id} className={`service-row group border-b border-black/15 py-5 sm:py-6 md:px-6 lg:py-5 ${index % 2 === 0 ? "md:border-r" : ""}`}>
                  <div className="flex items-start justify-between gap-3 sm:gap-5">
                    ...
                    <p className="shrink-0 whitespace-nowrap text-right text-base font-bold tabular-nums sm:text-lg">{service.price}</p>
                  </div>
                </article>
              ))}
            </div>
            ...
            <div className="grid gap-6 sm:gap-10 lg:grid-cols-[0.72fr_1.28fr] lg:gap-14">
              ...
              <div className="grid gap-px bg-white/15 sm:grid-cols-2">
                {benefits.map(([title, text], index) => (
                  <article key={title} className="bg-[#1a1815] p-5 sm:p-6">
        ...
        <section className="process-section text-[#f8ecdc]">
          <div className="mx-auto max-w-[1440px] px-4 py-10 sm:px-8 sm:py-16 lg:px-12 lg:py-14">
            <div className="flex flex-col justify-between gap-3 sm:flex-row sm:gap-5 sm:items-end">
              <div>
            ...
            <div className="mt-6 grid border-t border-white/25 sm:mt-8 md:grid-cols-3">
              {steps.map(([number, title, text], index) => (
                <article key={number} className={`process-step py-5 md:px-6 md:py-7 lg:px-8 ${index < 2 ? "border-b border-white/20 md:border-b-0 md:border-r" : ""}`}>
                  <div className="flex items-center gap-4">
                    <span className="font-display text-2xl font-black text-[#d7ae77] sm:text-3xl">{number}</span>
                    <span className="h-px flex-1 bg-white/20" aria-hidden="true" />
                  </div>
                  <h3 className="font-display mt-5 text-xl font-bold uppercase leading-tight sm:mt-6 sm:text-2xl lg:mt-7">{title}</h3>
        ...
        <section id="contacts" className="bg-[#efe7d8] text-[#1b1916]">
          <div className="mx-auto grid max-w-[1440px] gap-6 px-4 py-10 sm:gap-10 sm:px-8 sm:py-16 lg:grid-cols-[1.1fr_0.9fr] lg:px-12 lg:pb-12 lg:pt-16">
            <div>
              ...
              <a href="#lead-form" className="cta-dark mt-5 h-12 px-6 text-xs sm:mt-7 sm:h-14 sm:px-7 sm:text-sm">Записаться онлайн <span aria-hidden="true">→</span></a>
            </div>
            <address className="grid content-start gap-5 border-t border-black/20 pt-5 not-italic sm:pt-7 md:grid-cols-3 md:gap-0 lg:grid-cols-1 lg:gap-6 lg:self-start lg:border-l lg:border-t-0 lg:pl-12 lg:pt-0">
              <div>
                <p className="text-xs font-bold uppercase text-[#8b765c]">Адрес</p>
                <p className="mt-2 text-lg">Екатеринбург, ул. Малышева, 51</p>
                <a className="contact-action" href="https://yandex.ru/maps/?text=Екатеринбург%2C%20улица%20Малышева%2C%2051" target="_blank" rel="noreferrer">Построить маршрут →</a>
              </div>
              <div className="md:border-l md:border-black/15 md:pl-6 lg:border-l-0 lg:pl-0">
                <p className="text-xs font-bold uppercase text-[#8b765c]">Телефон</p>
                <a className="contact-phone mt-2 text-lg" href="tel:+73430000000">+7 (343) 000-00-00</a>
                <a className="contact-action" href="tel:+73430000000">Позвонить →</a>
              </div>
              <div className="md:border-l md:border-black/15 md:pl-6 lg:border-l-0 lg:pl-0">
                <p className="text-xs font-bold uppercase text-[#8b765c]">Часы работы</p>
                <p className="mt-2 text-lg">Ежедневно, 10:00–22:00</p>
                <a className="contact-action" href="#lead-form">Записаться →</a>
              </div>
            </address>
          </div>
        </section>
